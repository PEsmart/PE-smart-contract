...
//        var_dump($groupList);exit();
    ...
    public function edit($ids = NULL)
    {
        if ($this->request->isPost()) {
            ...
            //当FC:USDT比例改变
            if (array_key_exists("exchange_fc_rate",$row)){
               $item= json_decode($row['exchange_fc_rate']);
               foreach ($item as $k=>$value){
                   $rate=round($k/$value,4);
                   $y=date('Y');
                   $m=date('m');
                   $d=date('d');
                   ...
                   $isChangToday=db('currency_change')->whereBetween('create_time',[$start,$end])->find();

                   if ($isChangToday){
                       if ($isChangToday['price']!=$rate){
                           db('currency_change')->where('id',$isChangToday['id'])->update(['price'=>$rate,'create_time'=>time()]);
                       }
                   }else{
                       db('currency_change')->insert(['price'=>$rate,'create_time'=>time()]);
                       db()->commit();
                   }
